...
def launch_app():
    st.set_page_config(
        page_title="Q.U.E.S.T. – Quick Understanding and Extraction of Structured Text",
        layout="wide",
    )
    ...
    with st.sidebar:
        ...
        if current_file_names != st.session_state.uploaded_file_names:
            # Files have changed - reset everything
            st.session_state.messages = []
            st.session_state.files_processed = False
            st.session_state.vector_database = VectorDatabase()
            ...
            with st.spinner("Processing documents..."):
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    for text in ocr_applied_texts:
                        st.session_state.vector_database.add_vectors(
                            text["detected_text"]
                        )
    ...
    if st.session_state.files_processed and uploaded_files:
        ...
        if prompt := st.chat_input("Ask a question about your documents"):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    search_results = st.session_state.vector_database.search_vector(
                        prompt
                    )
        ...
        if not st.session_state.messages:
            display_chat_message(
                "assistant",
                "Hello! I'm your PDF assistant. Please upload some documents to get started.",
            )
